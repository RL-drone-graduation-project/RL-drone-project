...
class MultiRotorEnv(gym.Env):
    def __init__(self, drone_id, wind=False, speed=1.0, ip='127.0.0.1', verbose=1, port=8000, seed=100, is_distort=True):
        ...
        self.observation_space = gym.spaces.Box(low=np.full(31, -1), high=np.full(31, -1), dtype=np.float32)
    ...
    def step(self, action: np.ndarray):
        ...
        cur_pwm = [0.5947, 0.5947, 0.5947, 0.5947]

        for i in range(4):
            cur_pwm[i] = self.prev_pwm[i] + action[i] * 0.002
            if cur_pwm[i] > 1:
                cur_pwm[i] = 1
            elif cur_pwm[i] < 0:
                cur_pwm[i] = 0
        ...
        for i in range(4) :
            rotor_state[i] = rotor[i]['torque_scaler']
        #print(f'PWM = {cur_pwm}')

...
class MultiRotorEnv(gym.Env):
    def __init__(self, drone_id, wind=False, speed=1.0, ip='127.0.0.1', verbose=1, port=8000, seed=100, is_distort=True):
        ...
        self.time_per_step = 0.02
        self.distortion = 0.0

        origin_position = np.array([47.641468, -122.140165, 122.932])
    ...
    def reset(self):
        ...
        self.multi_rotor_info.reset(self.init_position,pos_info, imu_data, gps_data, self.prev_pwm, rotor_state)
        _pos,_ = self.multi_rotor_info.get_sensor_data()
